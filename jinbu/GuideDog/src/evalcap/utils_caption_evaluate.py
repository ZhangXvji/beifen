...
def bbox_eval(res_file, outfile=None):
    """
    ...
    """
    if not outfile:
        outfile = op.splitext(res_file)[0] + '.bbox_eval.json'


    ious = 0.
    iou_per_class = [0., 0., 0., 0.]
    acc = 0.
    acc_per_class = [0., 0., 0., 0.]
    count_per_class = [0, 0, 0, 0]
    acc_dict = {
        'Vehicle': 0. ,
        'Pedestrian': 0. ,
        'Infrastructure':0. ,
    ...
    for i, row in enumerate(tsv_reader(res_file)):
        json_reader = json.loads(row[1])[0]
        gt_label = int(json_reader['class_gt'])

        if 'box_iou' in json_reader.keys():
            ious += float(json_reader['box_iou'])
            iou_per_class[gt_label] += float(json_reader['box_iou'])
        if 'class' in json_reader.keys() and 'class_gt' in json_reader.keys():
            pred_label = float(json_reader['class'])
            pred_correctness = float(pred_label == gt_label)
            acc += pred_correctness
            acc_per_class[gt_label] += pred_correctness
        count_per_class[gt_label] += 1

    if acc_per_class != [0, 0, 0, 0]:
        assert sum(count_per_class) == i+1
        miou = ious / sum(count_per_class)
        macc = acc / sum(count_per_class)
        iou_per_class = [iou_per_class[i]/count_per_class[i] if count_per_class[i]!=0 else 0 for i in range(len(iou_per_class))]
        acc_per_class = [acc_per_class[i]/count_per_class[i] if count_per_class[i]!=0 else 0 for i in range(len(acc_per_class))]
    else:
        miou = ious / (i+1)
        iou_per_class = [iou_per_class[i]/count_per_class[i] if count_per_class[i]!=0 else 0 for i in range(len(iou_per_class))]
        macc = 0.
    ...
    if not outfile:
        ...
        with open(outfile, 'w') as fp:
            json.dump({
                'bbox_mean_iou': miou,
                'Vehicle_iou': iou_per_class[0] ,
                'Pedestrian_iou': iou_per_class[1] ,
                'Infrastructure_iou': iou_per_class[2] ,
                'Cyclist_iou': iou_per_class[3] ,
                'class_mean_acc': macc,
                'Vehicle': acc_per_class[0] ,
                'Pedestrian': acc_per_class[1] ,
                'Infrastructure': acc_per_class[2] ,
                'Cyclist': acc_per_class[3] ,
                'num_count': count_per_class 
                }, fp, indent=4)
    return miou, macc
...
def class_eval(res_file, outfile=None):
    ...
    acc = 0.
    acc_per_class = [0., 0., 0., 0., 0.]
    count_per_class = [0, 0, 0, 0, 0]
    acc_dict = {
        '0': 0. ,
        '1': 0. ,
        '2': 0. ,
...
def level_bbox_eval(res_file, outfile=None):
    """
    res_tsv: TSV file, each row is [image_key, json format list of captions].
             Each caption is a dict, with fields "caption", "conf".
             or JSON file of coco style
    ...
    inpaint_ious = 0.
    for i, row in enumerate(tsv_reader(res_file)):
        all_ious = json.loads(row[1])[0]['box_iou'].strip("[").strip("]").split(",")
        raw_ious += float(all_ious[0])
        inpaint_ious += float(all_ious[1])

    raw_miou = raw_ious / (i+1)
    inpaint_miou = inpaint_ious / (i+1)

    if not outfile:
        print(raw_miou, inpaint_miou)
    else:
        with open(outfile, 'w') as fp:
            json.dump({'raw_bbox_mean_iou': raw_miou, 'inpaint_bbox_mean_iou': inpaint_miou}, fp, indent=4)
    return (raw_miou, inpaint_miou)
