...
class DRAMAVideoTransformer(torch.nn.Module):
    def __init__(self, args, config, swin, transformer_encoder):
        ...
        self.class_head = nn.Linear(self.img_feature_dim, 5)
    ...
    def forward(self, *args, **kwargs):
        ...
        # bbox head
        class_logits = self.class_head(vid_feats.mean(1))
        ...
        if class_logits is not None:
            class_loss = get_class_loss(class_logits, torch.nn.functional.one_hot(kwargs['label'], 5).float())
