...
from src.evalcap.utils_caption_evaluate import evaluate_on_coco_caption, two_cap_evaluate_on_coco_caption, bbox_eval, level_bbox_eval, class_eval
...
def train(args, train_dataloader, val_dataloader, model, tokenizer, training_saver, optimizer, scheduler):
    ...
    for iteration, (img_keys, batch, meta_data) in enumerate(train_dataloader):
        ...
        print(f"img_feats : {batch[0].shape}")
        ...
        if args.pred_bbox:
            ...
            loss = loss_class * args.loss_class_w 
...
def evaluate(args, val_dataloader, model, tokenizer, output_dir):
    ...
    if is_main_process():
        ...
        acc_result = 0.
        if args.pred_bbox:
            iou_result, acc_result = class_eval(predict_file, outfile=bbox_eval_file)
        else:
    ...
    def gen_rows():
        ...
        with torch.no_grad():
            for step, (img_keys, batch, meta_data) in tqdm(enumerate(test_dataloader)):
                ...
                all_gt_classes = batch[1]

                for img_key, class_pred, class_gt in zip(img_keys, all_classes, all_gt_classes):
                    res = []
                    res.append({
