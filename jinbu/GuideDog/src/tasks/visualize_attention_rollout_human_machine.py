...
def inference(args, img_key, video_data, model, gt_bbox, class_label):
    ...
    grad_rollout = VITAttentionRollout(model, discard_ratio=0.9, head_fusion='mean', per_layer_per_head=True)
    ...
    mask, outputs = grad_rollout(inputs)
...
def main(args):
    ...
    raw_video_path = "data/human_machine_data"
    root_path =  "data/human_machine_data"
    ...
    for mode in ['test']:
        ...
        for idx in tqdm(range(len(filenames))):
            ...
            if 'class_preds' in outputs:
                all_classes = outputs["class_preds"][0]
            else:
                all_classes = -1*torch.ones_like(all_bboxs[0])

            save_path = "raw_attention_human_machine_v3_new"
            os.makedirs(save_path, exist_ok=True)

            frames = np.transpose(raw_frames, (0, 2, 3, 1))
            h, w = 256, 256

            raw_img = frames[0][:, :, ::-1] / 255
            raw_img = cv2.resize(raw_img, (h, w))

            all_images = []
            for layer_key in grayscale_cam.keys():
                layer_cam = grayscale_cam[layer_key]
                layer_images = []
                for head_key in layer_cam.keys():
                    cam_img = layer_cam[head_key]

                    # cam_array = cam_img.copy()
                    # np.save(os.path.join(save_path, filename.replace(".png", layer_head_key+".png")), cam_array)
                    cam_img = cv2.resize(cam_img, (h, w))

                    cam_image = show_cam_on_image(raw_img, cam_img)

                    gts = json.load(open("drama_bbox_gt.json", "r"))
                    if filename in gts:
                        gt_bbox = gts[filename]
                        gt_bbox = bbox_xywh2xyxy(gt_bbox)
                        cv2.rectangle(cam_image, (int(gt_bbox[0]*w), int(gt_bbox[1]*h)), (int(gt_bbox[2]*w), int(gt_bbox[3]*h)), (0, 0, 0), 4)
                    ...
                    layer_images.append(cam_image.copy())
                    
                all_images.append(np.hstack(layer_images))
            
            all_images = np.vstack(all_images)
            cv2.imwrite(os.path.join(save_path, filename), all_images)
