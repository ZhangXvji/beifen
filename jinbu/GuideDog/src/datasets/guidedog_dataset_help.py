...
class DogDataset(Dataset):
    def __init__(self):
        ...
        for folder in folders:
            ...
            if os.path.isdir(folder_path):
                folder_lidar = os.listdir(folder_path+"/video/0/")
    ...
    def __getitem__(self, index):
        ...
        videoL = self.get_video_tensor(bag_time, train_range, 8) # 3,60,56,56
    ...
    def get_video_tensor(self, bag_time, train_range, tag):
        if tag == 0:
            image_folder = f"dataset/{bag_time}/video/0/"
        elif tag == 2:
            image_folder = f"dataset/{bag_time}/video/2/"
        elif tag == 4:
            image_folder == f"dataset/{bag_time}/video/4/"
        elif tag == 6:
            image_folder == f"dataset/{bag_time}/video/6/"
        else:
            image_folder == f"dataset/{bag_time}/video/leg/"
    ...
    def get_label_tensor(self, bag_time, label_range):
        path = f"dataset/{bag_time}/ecparm/motor/motor_raw.csv"
...
#         print_hdf5_structure(file)
    ...
    data_list = [i for i in range(len(dataset))]
    random.shuffle(data_list)

    split_point = int(len(data_list) * 0.7)

    train_portion = data_list[:split_point]
    train_portion.sort()
    test_portion = data_list[split_point:]
