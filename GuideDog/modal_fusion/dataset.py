...
class DogDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        video0 = self.get_video_tensor(bag_time, train_range, 0) # 3,60,56,56
        video2 = self.get_video_tensor(bag_time, train_range, 2) # 3,60,56,56
        video4 = self.get_video_tensor(bag_time, train_range, 4) # 3,60,56,56
        video6 = self.get_video_tensor(bag_time, train_range, 6) # 3,60,56,56
        videoL = self.get_video_tensor(bag_time, train_range, "leg") # 3,60,56,56
    ...
    def get_video_tensor(self, bag_time, train_range, tag):
        if tag == 0 :
            image_folder = f"../dataset/raw/{bag_time}/video/0/"
        elif tag ==2 :
            image_folder = f"../dataset/raw/{bag_time}/video/2/"
        elif tag == 4 :
            image_folder = f"../dataset/raw/{bag_time}/video/4/"
        elif tag == 6 :
            image_folder = f"../dataset/raw/{bag_time}/video/6/"
        else:
    ...
    for index in range(len(dataset)):
        ...
        if sample['class'] == 1:
            if random.random() < 0.5:
                ...
                group.create_dataset('video0', data=sample['video0'])
                group.create_dataset('video2', data=sample['video2'])
                group.create_dataset('video4', data=sample['video4'])
                group.create_dataset('video6', data=sample['video6'])
                group.create_dataset('videoL', data=sample['videoL'])
