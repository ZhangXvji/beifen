...
import numpy as np
import multi_swin
from dataset_swim import DogDataset
...
class Args:
    def __init__(self) -> None:
        self.epochs, self.learning_rate, self.patience = [100, 1e-3, 99]
        self.batch_size = 1
...
for epoch in range(args.epochs):
    ...
    for idx, sample_batch in enumerate(train_dataloader, 0):
        optimizer.zero_grad()
        outputs = model(sample_batch['rgb0'].to(args.device),sample_batch['rgb2'].to(args.device),sample_batch['rgb4'].to(args.device),sample_batch['rgb6'].to(args.device), sample_batch['video'].to(args.device))
        # print(outputs.shape)
